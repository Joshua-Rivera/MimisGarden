from fastapi import (
    APIRouter,
    Depends,
)  # Import APIRouter and Depends for creating API routes and handling dependencies
...
from app.schemas.prediction_schema import (
    PredictionResponse,
)  # Import the PredictionLogResponse schema
from app.schemas.review_schema import (
    ReviewCreate,
    ReviewResponse,
)  # Import the ReviewCreate and ReviewResponse schemas
...
@router.get(
    "/reviews", response_model=list[PredictionResponse]
)  # Define a GET endpoint for retrieving the review queue
def list_review_queue(
    db: Session = Depends(get_db),
):  # Define the function to handle the GET request, with a database session dependency
    """
    ...
    return get_review_queue(
        db
    )  # Call the service function to get the review queue and return the results


@router.post(
    "/reviews/{prediction_id}", response_model=ReviewResponse
)  # Define a POST endpoint for submitting a review
def submit_review(
    prediction_id: str, review_data: ReviewCreate, db: Session = Depends(get_db)
):  # Define the function to handle the POST request, with a database session dependency
    """
